...
export function SettingsScreen() {
  ...
  const handleUpdateAddress = async () => {
    try {
      ...
      await fetch(`${API_BASE}/profile/update_profile.php`, {
        method: 'POST',
        headers: {
                    ...
                    {!isEditingAddress ? (
                      <div className="p-4 bg-brand/5 border border-brand/10 rounded-2xl flex items-start gap-4">
                        ...
                        <button
                          ...
                          className="text-brand font-black text-[10px] uppercase tracking-widest bg-brand/5 px-3 py-2 rounded-lg hover:bg-brand/10 transition-colors"
                        >
                          Edit
                        </button>
                      </div>
                    ) : (
                      <div className="space-y-4 p-4 bg-brand/5 border border-brand/20 rounded-2xl">
                        <div className="space-y-1">
                          <label className="text-[10px] font-black text-brand/40 uppercase px-1">Street Address</label>
                          <input
                            type="text"
                            value={editedAddress}
                            onChange={(e) => setEditedAddress(e.target.value)}
                            className="w-full bg-white border border-brand/10 rounded-xl py-3 px-4 text-sm font-bold text-brand outline-none"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[10px] font-black text-brand/40 uppercase px-1">City</label>
                          <input
                            type="text"
                            value={editedCity}
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="flex-1 bg-brand text-white py-3 rounded-xl font-black text-xs uppercase tracking-wider"
                          >
                            {t('save')}
                          </button>
                          <button
                            onClick={() => setIsEditingAddress(false)}
                            className="flex-1 bg-white border border-brand/10 text-brand/40 py-3 rounded-xl font-black text-xs uppercase tracking-wider"
                          >
                            {t('cancel')}
                          </button>
                        </div>
                    ...
                    {[
                      ...
                      <div key={item.id} className="p-5 flex items-center justify-between border-b border-brand/5 last:border-0">
                        ...
                        <button
                          onClick={() => {
                            if (item.id === 'bookings') {
                              togglePushNotifications(!notifSettings.bookings);
                            } else {
                              setNotifSettings(prev => ({ ...prev, [item.id]: !prev[item.id] }));
                            }
                          }}
