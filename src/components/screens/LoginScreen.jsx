...
import { Shield, Zap, ChevronLeft, Phone, Lock, Apple, Globe } from "lucide-react";
      ...
      <div className="px-6 pb-20 pt-4">
        <div className="mt-4 mb-8">
          ...
          <p className="text-brand/60 font-medium mt-1">
            {step === 1 ? "Sign in to continue your journey" : `Enter the code sent to ${phoneNumber}`}
          </p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label className="block text-[11px] font-black text-brand/40 uppercase tracking-[0.1em] mb-2 px-1">
                {authMode === "technician" ? "Technician Mobile Number" : "Mobile Number"}
              </label>
              <div className="relative group">
                <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${step === 2 ? 'text-brand/20' : 'text-brand/30 group-focus-within:text-brand'}`}>
                  <Phone size={18} />
                </div>
                <input 
                  type="tel" 
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <div className="mt-4 flex justify-between px-1">
                    <button type="button" onClick={() => setStep(1)} className="text-[11px] font-bold text-brand hover:underline">Change Number</button>
                    <button type="button" className="text-[11px] font-bold text-brand/40">Resend in 0:30</button>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
          ...
          >
            {step === 1 ? "Send Security Code" : (authMode === "technician" ? "Access Dashboard" : "Login Securely")}
        ...
        {step === 1 && (
          <>
            <div className="flex items-center gap-4 my-8">
              <div className="flex-1 h-[1px] bg-brand/5" />
              <span className="text-[10px] font-black text-brand/20 uppercase tracking-widest">Or social login</span>
              <div className="flex-1 h-[1px] bg-brand/5" />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <button className="flex items-center justify-center gap-2 py-3.5 rounded-xl border border-brand/10 bg-white hover:bg-brand/5 transition-colors">
                <Globe size={18} className="text-brand" />
                <span className="text-xs font-bold text-brand">Google</span>
              </button>
              <button className="flex items-center justify-center gap-2 py-3.5 rounded-xl border border-brand/10 bg-white hover:bg-brand/5 transition-colors">
                <Apple size={18} className="text-brand" />
                <span className="text-xs font-bold text-brand">Apple</span>
              </button>
            </div>
          </>
        )}
